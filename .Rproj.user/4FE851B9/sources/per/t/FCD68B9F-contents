...
USED_UP <-  last(
  gaz_rendetlen$Value[nrow(gaz_rendetlen)] -
    gaz_rendetlen$Value[gaz_rendetlen$Date  < "2023-08-01 0:00"])
...
# silly idea, I added the sinus, now I subtract it, but averaged over x years
mod_temp <- lm( tact ~ I(sin(hours_dat/24*3.14159)) + ns(day_in_wint,df=5),temps_xtra)
# stuff you might wish to do
# mod_temp %>% effects::predictorEffects() %>% plot
# plot(mod)
temps_xtra <- cbind( temps_xtra, 
                     predict(mod_temp, 
                             newdata = temps_xtra, 
                             interval = "prediction") %>%
                       `colnames<-`(c( "splined_temps", 
                                       "splined_temps_min", 
                                       "splined_temps_max" )))
...
const_0 <- 1730 / 2500 #KEDV_HATRA / (sum(preds$gas_corr/24) - preduse_corr)
const_5 <- 1730*0.95 / 2500#KEDV_HATRA * .95 / (sum(preds$gas_corr/24) - preduse_corr)
const_10 <- 1730*0.9 / 2500#KEDV_HATRA * .90 / (sum(preds$gas_corr/24) - preduse_corr)
const_15 <- 1730*0.85 / 2500#KEDV_HATRA * .85 / (sum(preds$gas_corr/24) - preduse_corr)
const_20 <- 1730*0.8 / 2500#KEDV_HATRA * .80 / (sum(preds$gas_corr/24) - preduse_corr)
...
fig_4 <- preds_day %>%
  ...
  scale_x_date(breaks = "7 weeks", 
               limits = c(as.Date("2022-08-01"),
                          as.Date("2023-08-01"))) +
  labs( y = "Temperature (°C), daily mean")
